...
*******************************************************/
//This versions was developed to support the use of the seconde generation
// of CO2 sensors "Infrared CO2 Sensor V2.0"
...
unsigned long initialTime;                   //variable for current time (for scv file)
int date[] = { 1, 1, 2022 };                 //Day, Month, Year;  variable for current date (for scv file)
...
#define SENSOR_DATA_PIN   (3)   //CO2 Sensor PWM interface
#define INTERRUPT_NUMBER   digitalPinToInterrupt(SENSOR_DATA_PIN)   //CO2 interrupt number

//Used in interrupt for CO2, calculate pulse width variable
...
volatile enum CO2_STATUS CO2_status = Undefined; 
volatile float CO2_concentration = -1;        //variable for keeping 
...
void setup() {
  ...
  attachInterrupt(INTERRUPT_NUMBER, CO2interruptChange, CHANGE);
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (SD.begin(2)) {
        ...
        delay(500);
        if (setStartHour()) {  //chose the time and date
          delay(500);
          setStartDate();
        }
        break;
...
void openFile(bool append) {
  ...
  if (append) {
    ...
    if (dataFile) {
      ...
      for (int i = 0; i < sizeof activeSensors; i++) {
        if (activeSensors[i]) {                    //add header in csv file
          if (i != (sizeof(activeSensors) - 1)) {  //make sure i != 3
            dataFile.print(listSensors[i]);
            dataFile.print(SEMICOLON);
          } else {  //add PM 2.5 and 10.0 header
            dataFile.print(F("PM 1.0"));
            dataFile.print(SEMICOLON);
            dataFile.print(F("PM 2.5"));
            dataFile.print(SEMICOLON);
            dataFile.print(F("PM 10"));
            dataFile.print(SEMICOLON);
          }
        }
      }
      dataFile.println();  //change line
    }
  }
...
bool setStartHour() {
  ...
  byte cursorPlace = 1;  //to know where the cursor is on the LCD
  int lenghtH, lenghtM, lenghtS;
  ;                          //for counting the number of characters
  unsigned long amountTime;  //variable for how much time to add
  int hSDset, mSDset;
  ...
  lcd.clear();                    //clean the display before write
  lcd.setCursor(0, 0);            //set the LCD cursor position first column(0) and first line(0)
  lcd.print(F("Current time:"));  //message to show

  lcd.setCursor(0, 1);
  lcd.print(F("1-Set 2-Skip"));  //message to show
  ...
  while (true) {
    if (millis() - lastTime > refreshRate) {
      lastTime = millis();
      lcd.setCursor(1, 1);
      hSDset = (initialTime / 3600000);                     //converts milliseconds to hours
      mSDset = (initialTime - (3600000 * hSDset)) / 60000;  //converts milliseconds to minutes
      lcd.print(hSDset);                                    //shows the time in hours
      lcd.print(F("h:"));
      lcd.print(mSDset);  //shows the time in minutes
      lcd.print(F("m    "));

      lenghtH = log10(hSDset);  //counts the number of characters - 1
      lenghtM = log10(mSDset);  //counts the number of characters - 1
      ...
      switch (getButtons()) {
        ...
        case 2:                                                  //the  second button couting from the left is being pressed
          ...
          break;
        case 3:                                      //the down button is being pressed
          if (initialTime >= amountTime) {           //prevents variavle rollover
            initialTime = initialTime - amountTime;  //reduces the experiment duration
          }
          break;
        case 4:                                                                      //the up button is being pressed
          initialTime = initialTime + amountTime * (1 - (initialTime >= 86399000));  //increases the experiment duration  (86399000 = 23h59m59s this the max limit)
          break;
        case 5:                    //the right button is being pressed
          lcd.noBlink();           //makes the LCD stop blinking
          hourSetTime = millis();  //save when time input was given
          return true;
      }
    }
  }
...
void setStartDate() {
  ...
  byte cursorPlace = 1;                     //to know where the cursor is on the LCD
  byte lengthDay, lengthMonth, lengthYear;  //for counting the number of characters
  ...
  while (true) {
    if (millis() - lastTime > refreshRate) {
      lastTime = millis();
      lcd.setCursor(1, 1);
      lcd.print(date[0]);  //shows the time in hours
      lcd.print(F("/"));
      lcd.print(date[1]);  //shows the time in minutes
      lcd.print(F("/"));
      lcd.print(date[2]);  //shows the time in minutes
      lcd.print(F("     "));

      lengthDay = log10(date[0]);    //counts the number of characters - 1
      lengthMonth = log10(date[1]);  //counts the number of characters - 1
      lengthYear = log10(date[2]);   //counts the number of characters - 1

      switch (cursorPlace) {
        case 1:                             //cursor in h
          lcd.setCursor(lengthDay + 1, 1);  //set cursor to h positions
          break;
        case 2:                                           //cursor in m
          lcd.setCursor(lengthDay + 3 + lengthMonth, 1);  //set cursor to m positions
          break;
        case 3:                                                        //cursor in s
          lcd.setCursor(lengthDay + lengthMonth + lengthYear + 5, 3);  //set cursor to s positions
          break;
      }
      ...
      switch (getButtons()) {
        ...
        case 2:                                                  //the  second button couting from the left is being pressed
          ...
          break;
        case 3:                                                 //the down button is being pressed
          if (date[cursorPlace - 1] >= 2) {                     //prevents variavle rollover
            date[cursorPlace - 1] = date[cursorPlace - 1] - 1;  //reduces the experiment duration
          }
          break;
        case 4:                                                                                     //the up button is being pressed
          if (cursorPlace == 1) {                                                                   //if on days
            date[cursorPlace - 1] = date[cursorPlace - 1] + (1 - (date[cursorPlace - 1] >= 31));    //increases the experiment duration  (31 days is the max limit)
          } else if (cursorPlace == 2) {                                                            //if on months
            date[cursorPlace - 1] = date[cursorPlace - 1] + (1 - (date[cursorPlace - 1] >= 12));    //increases the experiment duration  (12 months is the max limit)
          } else {                                                                                  //if on years
            date[cursorPlace - 1] = date[cursorPlace - 1] + (1 - (date[cursorPlace - 1] >= 9999));  //increases the experiment duration  (9999 years is the max limit)
          }
          break;
        case 5:                                      //the right button is being pressed
          lcd.noBlink();                             //makes the LCD stop blinking
          dataFile = SD.open(fileName, FILE_WRITE);  //add date to SD file
          return;
      }
    }
  }
...
bool mainScreen() {
  ...
  unsigned long SDTimer = writeIntervalSD;  //make sure it starts saving right at the beggining
  unsigned long experimentTime;
  bool statusSD;
  byte button, lastButton;
  ...
  while (millis() - setupDuration < experimentDuration) {
    ...
    if (millis() - SDTimer > writeIntervalSD) {
      ...
      if (activeSensors[0]) {                           //CO2
        sensorData[0] = CO2_concentration;
      }
      ...
      if (dataFile) {
        ...
        //add date and time
        experimentTime = (millis() - (setupDuration - hourSetTime) + initialTime) / 1000;  // time in seconds          " - (setupDuration - hourSetTime)" = subtracting the time between the input of time and when the main program starts running
        hSD = (experimentTime / 3600);                                                     //converts milliseconds to hours
        mSD = (experimentTime - (3600 * (unsigned long)hSD)) / 60;                         //converts milliseconds to minutes  "(unsigned long)"  = to prevent variable overflow
        sSD = experimentTime % 60;
        dataFile.print(date[2]);
        dataFile.print(F("-"));
        dataFile.print(date[1]);
        dataFile.print(F("-"));
        dataFile.print(date[0]);
        dataFile.print(F(" "));  //save date
        dataFile.print(hSD);
        dataFile.print(F(":"));
        dataFile.print(mSD);
        dataFile.print(F(":"));
        dataFile.print(sSD);
        dataFile.print(SEMICOLON);  //save time
        ...
        for (byte i = 0; i < (sizeof(activeSensors)); i++) {
          if (activeSensors[i]) {
            ...
            if (i == (sizeof(activeSensors) - 1)) {
              dataFile.print(sensorData[(i + 1)]);
              dataFile.print(SEMICOLON);
              dataFile.print(sensorData[(i + 2)]);
              dataFile.print(SEMICOLON);
            }
          }
        }
...
void CO2interruptChange() {  //helps read the data form the CO2 V2 sensor by reading the size of an ON signal
  if (digitalRead(SENSOR_DATA_PIN)) {
    ...
    if (2 == CO2_flag) {
      ...
      if (pwmHighVal_ms < 0.01) {
        ...
        CO2_status = Preheating;
      } else if (pwmHighVal_ms < 998.00) {
        CO2_status = Ok;
        CO2_concentration = (pwmHighVal_ms - 2) * 5;
      } else {
        CO2_status = Overload;
      }
