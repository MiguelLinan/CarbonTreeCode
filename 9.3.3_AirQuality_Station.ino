...
void setup() {
  ...
  while (true) {
    ...
    if (!mainScreen()) {      //if the person does not want to try another experiment: end
      ...
      lcd.print(F("SESSION"));
      lcd.setCursor(1, 1);
      lcd.write(byte(4));  lcd.print(F(" COMPLETED "));  lcd.write(byte(4));
      return;
    }
  }
...
bool setStartHour() {
  ...
  byte cursorPlace = 1;          //to know where the cursor is on the LCD
  int lenghtH, lenghtM, lenghtS;;                   //for counting the number of characters
  unsigned long  amountTime;               //variable for how much time to add
  int hSDset, mSDset;
  ...
  while (millis() - setupDuration < experimentDuration) {
    ...
    if (millis() - SDTimer > writeIntervalSD) {
      ...
      if (dataFile) {
        ...
        dataFile.print(date[2]); dataFile.print(F("-")); dataFile.print(date[1]); dataFile.print(F("-")); dataFile.print(date[0]); dataFile.print(F(" "));   //save date
        dataFile.print(hSD);  dataFile.print(F(":"));  dataFile.print(mSD);  dataFile.print(F(":"));  dataFile.print(sSD); dataFile.print(SEMICOLON);         //save time

        for (byte i = 0; i < (sizeof(activeSensors)); i++) {
          if (activeSensors[i]) {
            dataFile.print(sensorData[i]);  dataFile.print(SEMICOLON);
            if (i == (sizeof(activeSensors) - 1)) {
              dataFile.print(sensorData[(i + 1)]);  dataFile.print(SEMICOLON);
              dataFile.print(sensorData[(i + 2)]); dataFile.print(SEMICOLON);
            }
          }
        }
