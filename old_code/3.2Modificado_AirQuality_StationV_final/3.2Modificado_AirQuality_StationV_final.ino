...
void setup() {
  ...
  pinMode(SDPin, OUTPUT);                //start the SD library and card
...
void loop() {
  ...
  if ((millis() - ecra1Timer > intervalo) && ecra1) {
    ...
    if (CO2_Volt < 400) {    // era suposto ser else if 
      // print the results to the lcd
      lcd.print("CO2: ");                             //message to show
  ...
  if ((millis() - ecra4Timer > intervalo) && ecra4) {
    ...
    if (SD.begin()) {                                  //if file opens well then:
      SDFoundfile = true;
    }
    else {
      SDFoundfile = false;
    ...
    if (SDFoundfile && SDWriting) {                  //check if it has permition to write and if it found the file to write to
