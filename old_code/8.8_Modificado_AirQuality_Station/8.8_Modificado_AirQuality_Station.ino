...
float CO_ppm;                           //variable to store the CO value

#define LENG 31                          //0x42 + 31 bytes equal to 32 bytes
unsigned char buf[LENG];

float PM01Value = 0;                      //define PM1.0 value of the air detector module
float PM2_5Value = 0;                     //define PM2.5 value of the air detector module
float PM10Value = 0;                      //define PM10 value of the air detector module
...
void setup() {
  ...
  if (SD.begin()) {                                      //checking if SDreader and SDcard are connected and start the SD library
    ...
    mI = (writeIntervalSD - (3600000 * hI)) / 60000;                  //converts milliseconds to minutes
    sI = ((writeIntervalSD - (3600000 * hI)) - (mI * 60000)) / 1000;  //converts milliseconds to seconds
    StationData = SD.open("CTdata.TXT", FILE_WRITE);                                                                      /// ver iatooooooooo
